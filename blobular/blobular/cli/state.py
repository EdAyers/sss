...
class AppState(Current):
    ...
    def default(cls):
        ...
        db_path = cfg.local_cache_dir / "local.db"
        ...
        cloud_store = CloudBlobStore()
        blobstore = CacheBlobStore(SizedBlobStore( OnDatabaseBlobStore(result_table), local_file_store), cloud_store, table = cache_table)
        return cls(store= blobstore, local_file_store = local_file_store, cloud_store   = cloud_store)
